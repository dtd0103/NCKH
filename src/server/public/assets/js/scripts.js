...
async function updateCartPreview() {
    ...
    const subtotalElement = document.querySelector(".act-dropdown__row .act-dropdown__value");
    const totalElement = document.querySelector(".act-dropdown__row--bold .act-dropdown__value");
...
function updateTotals(cart) {
    ...
    for (const productId in cart) {
        if (cart.hasOwnProperty(productId)) {
            const quantity = cart[productId];
            const price = parseFloat(document.querySelector(`[data-id="${productId}"] .cart-item__price`).textContent.split(" ")[1]);
    ...
    const total = subtotal + shippingFee;
    document.querySelector(".cart-info__row span:nth-child(2)").textContent = totalQuantity;
    document.querySelector(".cart-info__subtotal").textContent = `$${subtotal.toFixed(2)}`;
    document.querySelector(".cart-info__shipping-fee").textContent = `$${shippingFee.toFixed(2)}`;
    document.querySelector(".cart-info__total").textContent = `$${total.toFixed(2)}`;
}

// Hàm hiển thị giỏ hàng trong trang shoppingcart.html
document.addEventListener("DOMContentLoaded", async function () {
    const cart = JSON.parse(localStorage.getItem("productIds")) || {};
            ...
            if (cart.hasOwnProperty(productId)) {
                    ...
                    <div class="cart-item__content">
                        <div class="cart-item__content-left">
                            ...
                            <p class="cart-item__price-wrap">
                                <span class="cart-item__price">Giá: ${product.SP_DonGia.toFixed(2)}đ</span> | 
                        ...
                        <div class="cart-item__content-right">
                            <p class="cart-item__total-price">$${productTotal.toFixed(2)}</p>
                ...
                decreaseBtn.addEventListener("click", function () {
                    if (cart[productId] > 1) {
                        ...
                        productElement.querySelector(".cart-item__total-price").textContent = 
                          `$${(product.SP_DonGia * cart[productId]).toFixed(2)}`;
                ...
                increaseBtn.addEventListener("click", function () {
                    if (cart[productId] < product.SP_SoLuong) {
                        ...
                        alert(`Không thể tăng số lượng vì chỉ còn ${product.SP_SoLuong} sản phẩm.`);
                    }
                });
...
// Gọi hàm cập nhật giỏ hàng ngay khi trang load
document.addEventListener("DOMContentLoaded", function () {
    updateCartPreview();
});
